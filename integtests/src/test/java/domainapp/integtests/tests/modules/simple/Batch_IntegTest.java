...
public class Batch_IntegTest extends DomainAppIntegTest {
    ...
    public static class Transfer extends Batch_IntegTest {
        ...
        public void happy_case() throws Exception {
            ...
            assertThat(batch.getVessels()).isEmpty();
            assertThat(fv.getState()).isEqualTo(FermentationVessel.State.READY_FOR_BATCH);
            assertThat(fv.getBatch()).isNull();

            // when
            wrap(batch).transfer(fv);
            transactionService.nextTransaction();

            // then
            assertThat(batch.getState()).isEqualTo(BatchState.FERMENTING);
            assertThat(batch.getVessels()).contains(fv);
            assertThat(fv.getState()).isEqualTo(FermentationVessel.State.FERMENTING);
            assertThat(fv.getBatch()).isEqualTo(batch);
        }

        @Test
        public void not_in_mashed_in_state() throws Exception {
